...
export async function recoveryAgent(
  ...
  switch (input.resistance_type) {
    case "task_too_large":
      return {
        micro_action: `Open a note for "${task}" and write only the first sub-step you can see.`,
        estimated_time: "3 minutes",
        estimated_minutes: 3,
        why_this_step: "It turns a broad task into one visible edge without requiring a full breakdown.",
        optional_next_step: "Pick the easiest listed sub-step and prepare it for two more minutes.",
        confidence: 0.9
      };
    ...
    case "fear_of_failure":
      return {
        micro_action: `Create a scratch version for "${task}" and label it "rough start" before adding one imperfect line.`,
        estimated_time: "4 minutes",
        estimated_minutes: 4,
        why_this_step: "It separates starting from quality judgment and lowers performance pressure.",
        optional_next_step: "Keep only the one line that seems usable.",
        confidence: 0.87
      };
    case "fear_of_difficulty":
      return {
        ...
        optional_next_step: "Mark one small section that now feels approachable.",
        confidence: 0.86
      };
    case "fatigue":
      return {
        ...
        optional_next_step: "If energy improves, stay for two more minutes on the easiest visible slice.",
        confidence: 0.92
      };
    default:
      return {
        ...
        optional_next_step: "Stay for one extra tiny action only if it still feels easy.",
        confidence: 0.85
      };
  }
}
